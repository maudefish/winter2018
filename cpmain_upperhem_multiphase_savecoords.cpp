...
#define INVERSE_SAMPLING  false 		/* random E, L, and B will be generated acc. to the real distributions */
...
int main() 
{
	...
	string source_fix = "source/";
	string diffuse_fix = "diffuse/";
	string e3source_fix = "e3source/";
	...
	vector<double> l_source = extract(inprefix + source_fix + "l_source");
	vector<double> b_source = extract(inprefix + source_fix + "b_source");
	...
	#if DIFFUSE == true
		cout << "\n\nDIFFUSE (MASKING): on\n\n";
		outprefix += diffuse_fix;
	#endif

	#if E3SOURCE == true
		cout << "\n\nUSING KNOWN SOURCES AS E3\n\n";
		outprefix += e3source_fix;
	#endif
